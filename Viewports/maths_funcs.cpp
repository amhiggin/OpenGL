...
/*
	Original version by Anton Gerdelan
	Amber Higgins, 13327954: authored the ortho function below.
	25th October 2016
*/
...
mat4 ortho(float left, float right, float bottom, float top, float near, float far){
	...
	float sx= 1/(right- left);//x scaling factor
	float sy=1 /(top-bottom);//y scaling factor
	float sz = 1 / (far - near);//z scaling factor
	
	//create a 4x4 matrix to store the values
	mat4 m = zero_mat4();

	//set matrix entries
	m.m[0] =2* sx;
	m.m[12]=-(right+left) *sx;
	m.m[5] = 2*sy;
	m.m[13] = -(top+bottom)*sy;
	m.m[10]=-2 *sz;
	m.m[14]=-(near+far)*sz;
	m.m[15]=1;
